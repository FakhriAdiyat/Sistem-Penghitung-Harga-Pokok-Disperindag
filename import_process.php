...
function setImportFlashAndRedirect(string $type, string $title, string $message, array $meta = []): void
{
...
if (!isset($_FILES['file_import'])) {
    setImportFlashAndRedirect('error', 'Import gagal', 'File tidak ditemukan. Silakan pilih file dan coba lagi.');
}
...
if (empty($rows)) {
    ...
    setImportFlashAndRedirect('error', 'Import gagal', $msg);
}
...
setImportFlashAndRedirect($type, $title, $message, ['success' => $success, 'failed' => $failed]);
